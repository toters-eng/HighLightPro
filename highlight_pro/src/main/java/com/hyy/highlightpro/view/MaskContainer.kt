            ...
            when (it) {
                ...
                Constraints.CenterHorizontalOfHighlight -> {
                    ...
                    if (resources.configuration.layoutDirection == ViewCompat.LAYOUT_DIRECTION_LTR && locale != "ku") {
                        ...
                        if (width <= 0) {
                            layoutParams.leftMargin =
                                (highLightRect.left - highLightRect.width() + highLightRect.width() / 2f).toInt()
                            gravities.add(Gravity.END)
                            view.doOnPreDraw { tipsView ->
                                layoutParams.leftMargin =
                                    (rootWidth - highLightRect.left - highLightRect.width() + highLightRect.width() / 2f - tipsView.width).toInt()
                                view.layoutParams = layoutParams
                            }
                        } else {
                            layoutParams.leftMargin =
                                (highLightRect.left - highLightRect.width() + highLightRect.width() / 2f - width).toInt()
                            gravities.add(Gravity.END)
                        }
                    }
                }
